...
// FootPrint 이미지 자동 슬라이드
let slideIndexFoot = 0;
showSlidesFootPrint();
function showSlidesFootPrint() {
    let i;
    let slides = document.getElementsByClassName("footprintImg");

    for (i = 0; i < slides.length; i++) {
        slides[i].style.display = "none";
    }
    slideIndexFoot++;
    if (slideIndexFoot > slides.length) {
        slideIndexFoot = 1;
    }
    slides[slideIndexFoot - 1].style.display = "block";

    setTimeout(showSlidesFootPrint, 2000); // 5초마다 이미지가 체인지됩니다
}  
